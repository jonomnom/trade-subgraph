...
import {
  ...
  AddLiquidity,
  RemoveLiquidity
} from "../generated/MvlpManager/MvlpManager"

import {
  Distribute
} from "../generated/FeeMvxRewardDistributor/RewardDistributor"
...
import {
  ...
  BuyUSDM as BuyUSDMEvent,
  SellUSDM as SellUSDMEvent,
...
export function handleIncreasePosition(event: IncreasePositionEvent): void {
  ...
  _storeActions(event, event.params.account.toHexString(), event.params.isLong? "IncreasePosition-Long": "IncreasePosition-Short", paramsString);
...
export function handleDecreasePosition(event: DecreasePositionEvent): void {
  ...
  _storeActions(event, event.params.account.toHexString(), event.params.isLong? "DecreasePosition-Long": "DecreasePosition-Short", paramsString);
}
...
export function handleBuyUSDM(event: BuyUSDMEvent): void {
  let volume = event.params.usdmAmount * BigInt.fromString("1000000000000")
  ...
  ',"tokenAmount":"' +event.params.tokenAmount.toString() + '"' +
  ',"usdmAmount":"' +event.params.usdmAmount.toString() + '"' +
  '}'
  
  _storeActions(event, event.params.account.toHexString(), "BuyUSDM", paramsString);
}

export function handleSellUSDM(event: SellUSDMEvent): void {
  let volume = event.params.usdmAmount * BigInt.fromString("1000000000000")
  ...
  _storeActions(event, event.params.account.toHexString(), "SellUSDM", paramsString);
}
...
export function handleSwap(event: SwapEvent): void {
  ...
  let paramsString = '{'+
  '"account": "'+event.params.account.toHexString()+'"'+ 
  ',"tokenIn":"' +event.params.tokenIn.toHexString() + '"' + 
  ',"tokenOut":"' +event.params.tokenOut.toHexString() + '"' +  
  ',"amountIn":"' +event.params.amountIn.toString() + '"' +  
  ',"amountOut":"' +event.params.amountOut.toString() + '"' +  
  '}'
  
  _storeActions(event, event.params.account.toHexString(), "Swap", paramsString);
}
...
export function handleAddLiquidity(event: AddLiquidity): void {
  _storeMvlpStat(event.block.timestamp, event.params.mvlpSupply, event.params.aumInUsdm)
}
...
export function handleDistributeEthToMvx(event: Distribute): void {
  let amount = event.params.amount
  let amountUsd = getTokenAmountUsd(WETH, amount)
  let totalEntity = _getOrCreateMvxStat("total", "total")
  ...
  let entity = _getOrCreateMvxStat(id, "daily")
...
export function handleDistributeEsmvxToMvx(event: Distribute): void {
  let amount = event.params.amount
  let amountUsd = getTokenAmountUsd(MVX, amount)

  let totalEntity = _getOrCreateMvxStat("total", "total")
  totalEntity.distributedEsmvx += amount
  totalEntity.distributedEsmvxCumulative += amount
  totalEntity.distributedEsmvxUsd += amountUsd
  totalEntity.distributedEsmvxUsdCumulative += amountUsd
  ...
  entity.distributedEsmvx += amount
  entity.distributedEsmvxCumulative = totalEntity.distributedEthCumulative
  entity.distributedEsmvxUsd += amountUsd
  entity.distributedEsmvxUsdCumulative = totalEntity.distributedUsdCumulative
...
function _getOrCreateMvxStat(id: string, period: string): MvxStat {
  let entity = MvxStat.load(id)
  if (entity == null) {
    entity = new MvxStat(id)
    ...
    entity.distributedEsmvx = ZERO
    entity.distributedEsmvxCumulative = ZERO
    entity.distributedEsmvxUsd = ZERO
    entity.distributedEsmvxUsdCumulative = ZERO
    entity.period = period
  }
  return entity as MvxStat
}
...
function _storeVolume(type: string, timestamp: BigInt, volume: BigInt): void {
  let periodHourTimestamp = parseInt(_getHourId(timestamp)) as i32
  let idHour = periodHourTimestamp.toString() + ":hourly"
  let entityHour = _getOrCreateVolumeStat(idHour, "hourly", periodHourTimestamp)
  entityHour.setBigInt(type, entityHour.getBigInt(type) + volume)
  entityHour.save()
...
function _getOrCreateMvlpStat(id: string, period: string, periodTimestmap: i32): MvlpStat {
  let entity = MvlpStat.load(id)
  if (entity == null) {
    entity = new MvlpStat(id)
    entity.period = period
    entity.mvlpSupply = ZERO
    entity.aumInUsdm = ZERO
    entity.distributedEth = ZERO
    entity.distributedEthCumulative = ZERO
    entity.distributedUsd = ZERO
    entity.distributedUsdCumulative = ZERO
  ...
  return entity as MvlpStat
}

export function handleDistributeEthToMvlp(event: Distribute): void {
  let amount = event.params.amount
  let amountUsd = getTokenAmountUsd(WETH, amount)
  let periodTimestamp = parseInt(_getDayId(event.block.timestamp)) as i32

  let totalEntity = _getOrCreateMvlpStat("total", "total", periodTimestamp)
  totalEntity.distributedEth += amount
  totalEntity.distributedEthCumulative += amount
  totalEntity.distributedUsd += amountUsd
  totalEntity.distributedUsdCumulative += amountUsd
  ...
  let id = periodTimestamp.toString() + ":daily"
  let entity = _getOrCreateMvlpStat(id, "daily", periodTimestamp)
...
export function handleDistributeEsmvxToMvlp(event: Distribute): void {
  let amount = event.params.amount
  let amountUsd = getTokenAmountUsd(MVX, amount)
  let periodTimestamp = parseInt(_getDayId(event.block.timestamp)) as i32
...
function _storeMvlpStat(timestamp: BigInt, mvlpSupply: BigInt, aumInUsdm: BigInt): void {
  let periodTimestamp = parseInt(_getDayId(timestamp)) as i32
  let totalEntity = _getOrCreateMvlpStat("total", "total", periodTimestamp)
  totalEntity.aumInUsdm = aumInUsdm
  totalEntity.mvlpSupply = mvlpSupply
  totalEntity.save()

  let id = periodTimestamp.toString() + ":daily"
  let entity = _getOrCreateMvlpStat(id, "daily", periodTimestamp)
  entity.aumInUsdm = aumInUsdm
  entity.mvlpSupply = mvlpSupply
  entity.save()
}

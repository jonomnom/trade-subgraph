...
export function _storeActions(event: ethereum.Event, account: string, actionName: string, params: string): void{

  let data = new ActionData(event.transaction.hash.toHexString()+"-"+actionName);
  data.account = account;
  data.action = actionName;
  data.params = params;
  data.blockNumber = event.block.number;
  data.txhash = event.transaction.hash.toHexString();
  data.timestamp = event.block.timestamp.toString();  

  data.save();

  let action = new Action(event.transaction.hash.toHexString()+"-"+actionName);
  action.data = data.id;
  action.save();
